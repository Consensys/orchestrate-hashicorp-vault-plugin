...
func (c *controller) importHandler() framework.OperationFunc {
	return func(ctx context.Context, req *logical.Request, data *framework.FieldData) (*logical.Response, error) {
		...
		if id == "" {
			return errors.WriteHTTPError(errors2.InvalidFormatError("id must be provided"))
		}
		if curve == "" {
			return errors.WriteHTTPError(errors2.InvalidFormatError("curve must be provided"))
		}
		if algo == "" {
			return errors.WriteHTTPError(errors2.InvalidFormatError("algorithm must be provided"))
		}
		if privateKeyString == "" {
			return errors.WriteHTTPError(errors2.InvalidFormatError("privateKey must be provided"))
		}
